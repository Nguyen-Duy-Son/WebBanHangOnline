...
const ProductDetail = () => {
  ...
  const handleAddProduct = async () => {
    try {
      await addOrDeleteProductOfBasket(user.id, accessToken.token, id, "add");
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <div>
        ...
        <div className="product_item-detail flex">
          <div className="product_item-detail-img">
            <img className="w-1/5" src={product.image} alt={product.name} />
          </div>
          <div className="product_item-detail-content">
            ...
            <p>
              Nếu bạn có nhu cầu mua sản phẩm hãy nhẫn vào mua hàng để thêm hàng
              vào giỏ hàng nhé! Shop cảm ơn bạn nhiều !
            </p>
            <button
              className="buy-button buy_button-detail"
